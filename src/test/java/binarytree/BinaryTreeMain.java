...
public class BinaryTreeMain {
	...
	Vector<Comparable<Person>> objects = new Vector<Comparable<Person>>();
		...
		System.out.println("Step from first to last element");
		System.out.println(tree.getFirstElement());
		for(int i=0;i<= tree.countElements();i++){
			System.out.println(tree.getNextElement());
		}
		
		System.out.println("Step from first to last element");
		System.out.println(tree.getLastElement());
		for(int i=0;i<= tree.countElements();i++){
			System.out.println(tree.getPreviousElement());
		}
		...
		/*
		for(Comparable<Person> o : objects){
			boolean ok = tree.findElement(o);
			if(ok){
				System.out.println(tree.getCurrentElement());
				tree.remove();
				System.out.println(tree.print());
			} else {
